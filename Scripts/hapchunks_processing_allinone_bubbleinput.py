...
def counts_per_haplotype_all_contigs (hap_records, bubbles_file):
	haplotype_records = open(hap_records, 'r')
	with open(bubbles_file) as f:
		data = f.read()
	nonrep_bubbles = ast.literal_eval(data)
	#for b in nonrep_bubbles:
	#	print (b)
	#	print(nonrep_bubbles[b])
	#nonrep_bubbles = json.loads(open(bubbles).replace("\'", "\""))
		...
		if 'chrom' not in haplotype_info[0] : #skip the header
			...
			for t in nonrep_bubbles.keys():
				...
				if t[0] in nodes_read or t[1] in nodes_read:
					#print(t)
					#print(nonrep_bubbles[t])
					print(sample, ' ', haplotype, 'new bubble', t, ' with inside nodes ', nonrep_bubbles[t])
					#print(t[0],'\t','nonrep_bubble_end')
					#print(t[1],'\t','nonrep_bubble_end')

					insides = flatten([nonrep_bubbles[t]])
					#if len(insides) > 1: #to exclude cases where there is just one inside node e.g. an insertion / deletion
					for n in insides:
						if n in nodes_read:
			...
			#to make sure no node is present in one but also shown as absent in the other dict because of some bubble nodes in different direction as compared to the rest
...
if __name__== "__main__":
	...
	counts_record, broken_contigs, unbroken_contigs = counts_per_haplotype_all_contigs(args.haps, args.bubbles)
